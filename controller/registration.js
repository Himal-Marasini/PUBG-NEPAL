...
const sendFreeMatchMail = require("../middleware/sendFreeMatchMail");
const Match = require("../model/CreateMatch");
const FreeRegistration = require("../model/FreeRegistration");
...
exports.getFreeMatch = async (req, res, next) => {
  const { id } = req.params;
  ...
  if (match.isFree != true) {
    return res.redirect("/");
  }

  return res.render("freeMatch-RegisterForm", {
    matchType: `${match.type}(${match.device})`,
    id: match._id
  });
};
...
exports.postFreeMatch = async (req, res, next) => {
  try {
    ...
    if (match.players.length > 98) {
      return res.json({
        status: false,
        message: "Lobby has been full !! Please check different matches "
      });
    }

    const user = new FreeRegistration({
      emailId: req.body.registrator_emailId,
      matchType: req.body.registrator_matchType,
    ...
    // At the send, Send Email
    const slot = match.players.length + 1;
    await sendFreeMatchMail(user, slot, res);
